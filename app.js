...
function dashboardHTML(s){
  ...
  return `<div class="kpis">${kpi('Total Parts',s.totalUnits,'All units in inventory','box',false,"goInventory('All')")}${kpi('Low Stock',s.lowCount,'Parts need restocking','warning',s.lowCount>0,"goInventory('Low')")}${kpi('Jobs Today',s.todayJobs,'Jobs logged today','clipboard',false,"goJobs()")}${kpi('Parts Used Today',s.partsUsedToday,'Total parts used','wrench',false,"goJobs()")}${kpi('Inventory Cost Value',money(s.costValue),'Current buying cost value','chart',false,"go('reports')")}</div>
...
function usedHTML(){
  ...
  matchingJobs.forEach(j=>{
    ...
    if(j.lines.length){
      j.lines.forEach(l=>usage.push({...l, date:j.date, plate:j.plate, car:j.car, job:j.description, doneBy:j.doneBy, status:jobStatus(j), customText, labour:j.labour, total:j.total, jobId:j.id}));
    } else {
      usage.push({name:'No inventory part', sku:'', qty:0, price:0, date:j.date, plate:j.plate, car:j.car, job:j.description, doneBy:j.doneBy, status:jobStatus(j), customText, labour:j.labour, total:j.total, jobId:j.id});
    }
  });
  ...
  <section class="panel" style="margin-top:20px"><div class="section-title"><h3>${plateQuery ? `Jobs Matching ${esc(plateQuery)}` : 'Completed Jobs'}</h3><button class="mini-btn" onclick="go('job')">Log New Job</button></div>${matchingJobs.length?`<div class="jobs-list">${matchingJobs.map(j=>`<div class="job-row"><div><b>${esc(j.plate)}</b><small class="muted">${esc(j.car)} · ${esc(j.date)}</small></div><div><b>${esc(j.description)}</b><small class="muted">Status: ${esc(jobStatus(j))} · Done by: ${esc(j.doneBy || '—')} · ${j.lines.map(l=>`${esc(l.name)} x ${l.qty}`).join(', ') || 'No parts used'}</small></div><div><button class="mini-btn" onclick="viewPlateHistory('${esc(j.plate)}')">Plate History</button> <button class="mini-btn" onclick="viewJob('${j.id}')">Open</button> <button class="mini-btn danger" onclick="deleteJob('${j.id}')">Delete / Return Stock</button></div></div>`).join('')}</div>`:'<div class="empty">No jobs found.</div>'}</section>
  <section class="panel" style="margin-top:20px"><div class="section-title"><h3>${plateQuery ? `Part Usage for ${esc(plateQuery)}` : 'Part Usage Log'}</h3></div>${usage.length?`<div class="table-wrap"><table><thead><tr><th>Date</th><th>Plate</th><th>Car</th><th>Job</th><th>Status</th><th>Done By</th><th>Part</th><th>Qty</th><th>Item Price</th><th>Custom Charge</th><th>Labor Charge</th><th>Total</th></tr></thead><tbody>${usage.map(u=>`<tr><td>${esc(u.date)}</td><td>${esc(u.plate)}</td><td>${esc(u.car)}</td><td>${esc(u.job)}</td><td>${esc(u.status || 'Done Completed')}</td><td>${esc(u.doneBy || '—')}</td><td>${esc(u.name)}<br><small class="muted">${esc(u.sku)}</small></td><td>${u.qty}</td><td>${money(u.price)}</td><td>${u.customText?esc(u.customText):money(0)}</td><td>${money(u.labour)}</td><td>${money(u.total)}</td></tr>`).join('')}</tbody></table></div>`:'<div class="empty">No parts used yet.</div>'}</section>`;
}
function reportsHTML(s){
  ...
  jobs.forEach(j=>{
    ...
    if(j.lines.length){
      j.lines.forEach(l=>partRows.push({date:j.date, plate:j.plate, car:j.car, job:j.description, status:jobStatus(j), doneBy:j.doneBy, name:l.name, sku:l.sku, qty:l.qty, itemPrice:Number(l.price||0), customText, customTotal, labour:Number(j.labour||0), total:Number(j.total||0)}));
    } else {
      partRows.push({date:j.date, plate:j.plate, car:j.car, job:j.description, status:jobStatus(j), doneBy:j.doneBy, name:'No inventory part', sku:'', qty:0, itemPrice:0, customText, customTotal, labour:Number(j.labour||0), total:Number(j.total||0)});
    }
  });
  ...
  <section class="panel" style="margin-top:20px"><div class="section-title"><h3>Monthly Parts Used</h3></div>${partRows.length?`<div class="table-wrap"><table><thead><tr><th>Date</th><th>Plate</th><th>Car</th><th>Job</th><th>Status</th><th>Done By</th><th>Part</th><th>Qty</th><th>Item Price</th><th>Custom Charge</th><th>Labor Charge</th><th>Total</th></tr></thead><tbody>${partRows.map(r=>`<tr><td>${esc(r.date)}</td><td>${esc(r.plate)}</td><td>${esc(r.car)}</td><td>${esc(r.job)}</td><td>${esc(r.status || 'Done Completed')}</td><td>${esc(r.doneBy || '—')}</td><td>${esc(r.name)}<br><small class="muted">${esc(r.sku)}</small></td><td>${r.qty}</td><td>${money(r.itemPrice)}</td><td>${r.customText?esc(r.customText):money(0)}</td><td>${money(r.labour)}</td><td>${money(r.total)}</td></tr>`).join('')}</tbody></table></div>`:'<div class="empty">No jobs in this month yet.</div>'}</section>
...
function exportDashboardJobs(){
  const rows=[['Date','Status','Done By','Plate','Car','Customer','Job','Parts Used','Custom Charges AED','Labor Hours','Labor Charge AED','Total AED']];
  state.jobs.slice(0,4).forEach(j=>{
    ...
    rows.push([j.date,jobStatus(j),j.doneBy||'',j.plate,j.car,j.customer,j.description,parts,custom,j.labourHours||0,j.labour||0,j.total||0]);
  });
...
function exportUsage(){
  const rows=[['Date','Status','Done By','Plate','Car','Job','Part','SKU','Qty','Item Price AED','Custom Charge','Custom Charge AED','Labor Charge AED','Total AED']];
  state.jobs.forEach(j=>{
    const customNames=(j.customCharges||[]).map(c=>c.name).join('; ');
    const customTotal=(j.customCharges||[]).reduce((a,c)=>a+Number(c.amount||0),0);
    if(j.lines.length){
      j.lines.forEach(l=>rows.push([j.date,jobStatus(j),j.doneBy||'',j.plate,j.car,j.description,l.name,l.sku,l.qty,l.price,customNames,customTotal,j.labour,j.total]));
    } else {
      rows.push([j.date,jobStatus(j),j.doneBy||'',j.plate,j.car,j.description,'','',0,0,customNames,customTotal,j.labour,j.total]);
    }
  });
...
function exportMonthlyReport(){
  const month = state.reportMonth || today().slice(0,7);
  const rows=[['Date','Status','Done By','Plate','Car','Job','Part','Qty','Item Price AED','Custom Charge','Custom Charge AED','Labor Charge AED','Total AED']];
  state.jobs.filter(j => (j.date || '').slice(0,7) === month).forEach(j=>{
    const customNames=(j.customCharges||[]).map(c=>c.name).join('; ');
    const customTotal=(j.customCharges||[]).reduce((a,c)=>a+Number(c.amount||0),0);
    if(j.lines.length){ j.lines.forEach(l=>rows.push([j.date,jobStatus(j),j.doneBy||'',j.plate,j.car,j.description,l.name,l.qty,l.price,customNames,customTotal,j.labour,j.total])); }
    else rows.push([j.date,jobStatus(j),j.doneBy||'',j.plate,j.car,j.description,'',0,0,customNames,customTotal,j.labour,j.total]);
  });
